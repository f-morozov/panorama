...
namespace boostDesc
{
 ...
 {
  public:
  BGM (string _wlFile = "../binboost/matrices/bgm.bin") :
        wlFile(_wlFile)
   {
   ...
   virtual int defaultNorm() const {return 1;}

   virtual void computeImpl( InputArray image, std::vector<KeyPoint>& keypoints, OutputArray descriptors ) const 
   {
      Mat img = image.getMat();
      descriptors.create(keypoints.size(), ceil(nDim/8), descriptorType());
      Mat dst = descriptors.getMat();
      compute(img, keypoints, dst);
   }
 ...
 {
  public:
  LBGM (string _wlFile = "../binboost/matrices/lbgm.bin")
   {
    wlFile = _wlFile;
 ...
 {
  public:
   BinBoost (string _wlFile = "../binboost/matrices/binboost.bin") :
